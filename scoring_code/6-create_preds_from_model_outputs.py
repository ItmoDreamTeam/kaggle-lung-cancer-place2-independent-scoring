...
def process_ens2_sg1(names, labels):
    ...
    df_masses = pd.read_csv('old_res_files/stage1_masses_predictions.csv')
    df_masses = df_masses.rename(columns={'patient_id': 'id', 'prediction': 'mass_pred'})

    dfs_sg1 = []

    for name in names:
        dfs_sg1.append(pd.read_csv('old_res_files/model_features_stage1_' + name + '.csv'))
...
def process_ens1_sg1(labels):
    df_ens1 = pd.read_csv("old_res_files/weighted_ensemble_v1_nodulesv29_stage1.csv")
...
def process_jul():
    ...
    jul_fs_trn = pd.read_csv(r"old_res_files/train_luna16_fs.csv")
    jul_fs_test = pd.read_csv("old_res_files/submission_luna16_fs.csv")
    ...
    jul_dsb1_trn = pd.read_csv("old_res_files/train_luna_posnegndsb_v1.csv")
    jul_dsb1_test = pd.read_csv("old_res_files/submission_luna_posnegndsb_v1.csv")
    assert all(jul_fs_trn.columns == jul_fs_test.columns)
    ...
    jul_dsb2_trn = pd.read_csv("old_res_files/train_luna_posnegndsb_v2.csv")
    jul_dsb2_test = pd.read_csv("old_res_files/submission_luna_posnegndsb_v2.csv")
    assert all(jul_fs_trn.columns == jul_fs_test.columns)
