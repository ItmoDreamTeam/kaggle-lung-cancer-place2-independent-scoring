...
OUTPUT_BASE_PATH = configuration.TMP_DIR + '/v29_nodules'
...
def get_fuzzed_view(img, center, n_view, VOXEL_SIZE):
    ...
    for i in range(n_view):
        ...
        # pdb.set_trace()
        voxels.append(img[fuzzed_center[0] - halfsize:fuzzed_center[0] + halfsize,
                      fuzzed_center[1] - halfsize:fuzzed_center[1] + halfsize,
                      fuzzed_center[2] - halfsize:fuzzed_center[2] + halfsize])
...
def get_subvox_at(img, center, size=64):
    subimg = img[center[0] - size / 2:center[0] + size / 2, center[1] - size / 2:center[1] + size / 2,
...
def rough_malig_score(preds, diam_ix=0, lob_ix=1, malig_ix=3):
    return scipy.special.expit(
        -9.5 + preds[:, diam_ix] * .0008228 + preds[:, lob_ix] * 1.304 + preds[:, malig_ix] * 10.45)
...
if __name__ == '__main__':
    ...
    for i, file in enumerate(train_files):
        ...
        np.save(join(OUTPUT_BASE_PATH, 'shapes_' + file), np.array(shapes))
